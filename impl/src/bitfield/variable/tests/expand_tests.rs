...
fn test_expand_variable_struct_complete() {
    ...
    config
        .bits(BitsConfig::Variable(vec![32, 48, 64]), Span::call_site())
        .unwrap();
...
fn test_generate_variable_specifier_impl() {
    ...
    assert!(
        generated.contains("impl")
            && generated.contains("Specifier")
            && generated.contains("TestStruct")
    );
...
fn test_generate_variable_size_extensions() {
    ...
    let mut config = Config::default();
    config
        .bits(BitsConfig::Variable(vec![16, 32]), Span::call_site())
        .unwrap();
...
fn test_analyze_and_expand_integration() {
    ...
    let mut config = Config::default();
    config
        .bits(BitsConfig::Variable(vec![24, 32, 48]), Span::call_site())
        .unwrap();
