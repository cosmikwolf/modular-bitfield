...
use super::super::field_config_ext::{VariableFieldConfigExt, VariableFieldConfigs};
...
fn test_multiple_fields() {
    ...
    configs
        .set_variant_discriminator(2, Span::call_site())
        .unwrap();
    configs.set_variant_data(3, Span::call_site()).unwrap();

    // Verify each field
    assert_eq!(
        configs.get(0).unwrap().variant_role(),
        Some(VariantRole::Discriminator)
    );
    assert_eq!(
        configs.get(1).unwrap().variant_role(),
        Some(VariantRole::Data)
    );
    assert_eq!(
        configs.get(2).unwrap().variant_role(),
        Some(VariantRole::Discriminator)
    );
    assert_eq!(
        configs.get(3).unwrap().variant_role(),
        Some(VariantRole::Data)
    );
...
fn test_field_config_variant_role() {
    ...
    assert_eq!(
        field_config.variant_role(0, &configs),
        Some(VariantRole::Discriminator)
    );
    assert_eq!(
        field_config.variant_role(1, &configs),
        Some(VariantRole::Data)
    );
...
fn test_sparse_field_indices() {
    ...
    // Set sparse indices
    configs
        .set_variant_discriminator(10, Span::call_site())
        .unwrap();
    configs.set_variant_data(50, Span::call_site()).unwrap();
    configs
        .set_variant_discriminator(100, Span::call_site())
        .unwrap();
    ...
    assert_eq!(
        field_config.variant_role(10, &configs),
        Some(VariantRole::Discriminator)
    );
    assert_eq!(
        field_config.variant_role(50, &configs),
        Some(VariantRole::Data)
    );
    assert_eq!(
        field_config.variant_role(100, &configs),
        Some(VariantRole::Discriminator)
    );
