...
fn test_variable_enum_round_trip_all_variants() {
    ...
    for (original, expected_discriminant) in test_cases {
        ...
        let reconstructed =
            RoundTripTest::from_discriminant_and_bytes(expected_discriminant, bytes).unwrap();
...
fn test_variable_enum_with_different_data_types() {
    ...
    for variant in [flag, nibble, byte] {
        ...
        let reconstructed =
            TypeTest::from_discriminant_and_bytes(variant.discriminant(), bytes).unwrap();
...
fn test_variable_enum_consistent_with_fixed_enum() {
    ...
    assert_eq!(
        <FixedEnum as Specifier>::BITS,
        <VariableEnum as Specifier>::BITS
    );
